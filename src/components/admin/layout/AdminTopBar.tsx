  ...
  return (
    <>
      <header className="h-[68px] sm:h-[72px] bg-white border-b border-slate-200 sticky top-0 z-20 px-4 sm:px-6 flex items-center justify-between gap-3 shadow-xs">
        {/* Left Side: Mobile Menu, Desktop Collapse Toggle, and Breadcrumbs */}
        ...
        {/* Right Side: Selectors, Search, Notifications, Profile */}
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Financial Year Selector (Disabled - No Reporting Cycle) */}
          <div className="hidden xl:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-500" title="No reporting cycle configured in backend">
            ...
            >
              <option>No reporting cycle</option>
            </select>
          </div>

          {/* Organization Scope Selector (Disabled - No Org Selected) */}
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-500" title="No organization unit configured in backend">
            <Building className="w-3.5 h-3.5 text-slate-400 shrink-0" />
            <span className="font-semibold text-slate-600">Scope:</span>
            <select
              disabled
              className="bg-transparent text-slate-400 font-medium text-xs focus:outline-none cursor-not-allowed max-w-[150px] truncate"
            >
          ...
          <div className="relative" ref={profileRef}>
            ...
            >
              <div className="w-8 h-8 rounded-full bg-[#003B73] text-white flex items-center justify-center font-bold text-xs shadow-xs">
                EA
              </div>
              ...
              <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-4 py-2.5 border-b border-slate-100">
                  <p className="text-xs font-bold text-slate-900 truncate">
                    {session?.user?.name || 'Demo ESG Administrator'}
                  </p>
