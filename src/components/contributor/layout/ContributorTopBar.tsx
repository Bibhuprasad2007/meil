...
import { ContributorBreadcrumbs } from './ContributorBreadcrumbs';
      ...
      <header className="h-[68px] sm:h-[72px] bg-white border-b border-slate-200 sticky top-0 z-20 px-4 sm:px-6 flex items-center justify-between gap-3 shadow-xs">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="hidden sm:block truncate">
            <ContributorBreadcrumbs />
          </div>
        </div>

        {/* Right Side */}
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Reporting Cycle Indicator */}
          <div
          ...
          >
            ...
            <span className="text-slate-500 font-medium">No scope assigned</span>
          </div>
              ...
              <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-4 py-2.5 border-b border-slate-100">
                  <p className="text-xs font-bold text-slate-900 truncate">
                    {session?.user?.name || 'Demo Contributor'}
                  </p>
                  <p className="text-[11px] text-slate-500 truncate">
                    {session?.user?.email || 'demo.contributor@meil.test'}
                  </p>
                  <div className="mt-2 inline-flex items-center gap-1 text-[10px] font-semibold text-teal-800 bg-teal-50 px-2 py-0.5 rounded border border-teal-200">
